<template>
          ...
          <div class="order-content">
            ...
            <div class="orders">
              <table class="order-item" v-for="myOrder in myOrderList.records" :key="myOrder.id">
                <thead>
                  <tr>
                    <th colspan="5">
                      <span class="ordertitle">{{myOrder.createTime}}  订单编号：{{myOrder.outTradeNo}} <span
                  ...
                  <tr v-for="(cart,index) in myOrder.orderDetailList" :key="cart.id">
                    <td width="60%">
                      <div class="typographic">
                        <img :src="cart.imgUrl" style="width: 50px;heightL50px">
                        <a href="#" class="block-text">{{cart.skuName}}</a>
                        <span>x{{cart.skuNum}}</span>
                        <a href="#" class="service">售后申请</a>
                      </div>
                    </td>
                    <td :rowspan="myOrder.orderDetailList.length" width="8%" class="center" v-if="index === 0">{{myOrder.consignee}}</td>
                    ...
                    <td :rowspan="myOrder.orderDetailList.length" width="8%" class="center" v-if="index === 0">
            ...
            <div class="choose-order">
                <!-- 分页器 -->
                <pagination :pageNo='page' :pageSize='limit' :total='myOrderList.total' :continues='5' @getPageNo='getPageNo'/>
            </div>
          </div>
          <!--猜你喜欢-->
          <div class="like">
...
export default {
  ...
  computed: {
    ...mapState('MyOrder', {
      myOrderList: state => state.myOrder
    })
  },
  methods: {
    // 获取我的订单
    getData () {
      ...
      this.$store.dispatch('MyOrder/getMyOrder', { page, limit })
    },
    // 获取当前页
    getPageNo (page) {
      // 修改组件响应式数据
